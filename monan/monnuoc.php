...
$pageTitle = "Món Nước | Bếp Anh Tài";
...
$categoryTitle = "Món Nước";
$categoryDescription = "Khám phá các món nước giải khát thơm ngon, mát lạnh từ trà sữa, nước ép, sinh tố cho đến cà phê và đồ uống detox cực chất.";
$categorySlug = 'mon_nuoc'; // Slug này phải khớp với giá trị trong cột `category` của bảng `posts`
...
if ($conn) { // ĐÃ THÊM: Đảm bảo kết nối CSDL thành công
    ...
    if ($stmt) { // ĐÃ THÊM: Kiểm tra xem prepare có thành công không
        ...
        error_log("Prepare statement failed in monnuoc.php: " . $conn->error); // Ghi log lỗi
    }
} else {
    error_log("Database connection is not established in monnuoc.php"); // Ghi log lỗi
}
